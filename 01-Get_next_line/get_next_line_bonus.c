...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/23 11:32:43 by lle-saul          #+#    #+#             */
/*   Updated: 2023/10/23 14:30:53 by lle-saul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	line = NULL;
	size = 1;
	ft_read(&stock[fd], fd, &size);
	if (!stock[fd] || size < 0)
		return (NULL);
	line = ft_line(stock[fd]);
	if (!line)
		return (NULL);
	ft_clear_stock(&stock[fd]);
	if (line[0] == '\0')
	{
		ft_free_stock(stock[fd]);
		stock[fd] = NULL;
		free(line);
		return (NULL);
	}
